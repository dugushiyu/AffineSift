...
int compute_asift_matches(int num_of_tilts1, int num_of_tilts2, int w1, int h1, int w2, int h2, int verb, vector< vector< keypointslist > >& keys1, vector< vector< keypointslist > >& keys2, matchingslist &matchings, siftPar &siftparameters)
{
  ...
  unsigned num_procs = omp_get_num_procs();
  unsigned T1 = num_procs / 2;
  unsigned T2 = num_procs / 4;
  unsigned T3 = num_procs / 6;
  unsigned T4 = num_procs / 8;
  //
  if (T1 <= 0) T1 = 1;
  if (T2 <= 0) T2 = 1;
  if (T3 <= 0) T3 = 1;
  if (T4 <= 0) T4 = 1;
  vector<unsigned> thread_vec = {T1,T2,T3,T4};
  unsigned threads_num = *std::max_element(thread_vec.begin(), thread_vec.end());
  // loop on tilts for image 1.
  #pragma omp parallel for private(tt) num_threads(threads_num)
  for (int tt = 1; tt <= num_tilt1; tt++) {
    float t = t_min * pow(t_k, tt - 1);
